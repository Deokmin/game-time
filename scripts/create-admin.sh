...
# 최초 admin 계정 생성 스크립트
#
# Terraform 은 User Pool / Client 만 생성하며, admin 계정은 이 스크립트로
# 생성한다. (Terraform state 에 비밀번호가 저장되지 않도록 하기 위함)
#
# 동작:
#   1. 랜덤 임시 비밀번호 생성
#   2. aws cognito-idp admin-create-user 로 admin 계정 생성
...
usage() {
  ...
  echo "  -f  가족 ID (필수, 영문 소문자/숫자/하이픈 1~64자)"
...
if [[ -z "${USERNAME}" || -z "${FAMILY_ID}" ]]; then
  echo "[오류] -u <username> 과 -f <family_id> 는 필수입니다." >&2
  usage
fi
...
# ------------------------------------------------------------------------------
# 랜덤 임시 비밀번호 생성
# (Cognito 정책: 8자 이상, 대문자/소문자/숫자 포함 - 여기서는 16자 생성)
# ------------------------------------------------------------------------------

generate_password() {
  local upper lower digits rest
  # LC_ALL=C 로 바이트 단위 처리 보장, /dev/urandom 으로 예측 불가능성 확보
  upper="$(LC_ALL=C tr -dc 'A-HJ-NP-Z' < /dev/urandom | head -c 2)"
  lower="$(LC_ALL=C tr -dc 'a-km-z' < /dev/urandom | head -c 2)"
  digits="$(LC_ALL=C tr -dc '2-9' < /dev/urandom | head -c 2)"
  rest="$(LC_ALL=C tr -dc 'A-HJ-NP-Za-km-z2-9' < /dev/urandom | head -c 10)"
  # fold 로 한 글자씩 분리 후 shuf 로 섞어 위치 무작위화
  printf '%s' "${upper}${lower}${digits}${rest}" | fold -w1 | shuf | tr -d '\n'
}

TEMP_PASSWORD="$(generate_password)"

# ------------------------------------------------------------------------------
# 1. admin 계정 생성 (이메일/SMS 발송 억제)
# ------------------------------------------------------------------------------

echo "[정보] admin 계정을 생성합니다: ${USERNAME} (family_id=${FAMILY_ID})"
...
aws cognito-idp admin-create-user \
  ...
  --user-attributes \
    ...
    Name=custom:family_id,Value="${FAMILY_ID}" \
  > /dev/null
